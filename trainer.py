...
from algos import gem, ogd, sogd
from algos import sketch
import numpy as np
...
class Trainer(object):
    def __init__(self, config, val_loaders):
        ...
        #  form sketch and ogd_basis
        print("MODEL COUNT PARAMS: ",count_parameter(self.model))
        if self.config.is_split and (self.config.dataset != "split_mnist"):
            print("running split_cifar probably")
            n_params = parameters_to_vector(self.get_params_dict(last=False)).shape[0]
        else:
            n_params = count_parameter(self.model)
        print("n_params: ", n_params)
        self.ogd_basis = torch.empty(n_params, 0).to(self.config.device)
        self.target_rank = self.config.memory_size  # total amount of memory columns available. Should it be self.config.memory_size * self.config.n_tasks?

        # we choose not to use truncation so target rank is k
        if config.sketch_method == "basic":
            self.sketch = sketch.Sketch(n_params, 0, self.target_rank-2, self.target_rank, int(self.target_rank+2))
        else:
            self.sketch = sketch.Sketch(n_params, n_params, self.target_rank-2, self.target_rank, int(self.target_rank+2))  # n,n,r,k,l
        self.sketch_basis = torch.empty(n_params, 0).to(self.config.device)
        ...
        self.compute_singular_values = config.compute_singular_values
        self.unchanged_gradients = torch.empty((n_params,0)).cpu()
    ...
    def create_model(self):
        ...
        print("CONFIG.OUT_DIM: ", cfg.out_dim)
    ...
    def optimizer_step(self, first_task = False):
        ...
        if self.config.method in ['ogd', 'pca'] and not first_task:
        ...
        elif self.config.method=='sogd' and not first_task:
            proj_grad_vec = sogd.project_vec(grad_vec,
                                            proj_basis=self.sketch_basis)
            ## take the orthogonal projection
            new_grad_vec = grad_vec - proj_grad_vec
           
        elif self.config.method=="agem" and self.agem_mem_loader is not None and not first_task:
            self.optimizer.zero_grad()
            data, target, task = next(iter(self.agem_mem_loader))
            # datasets = self.to_device(datasets)
            ...
            if self.task_count >= 1:
                 ...
                 for t,mem in self.task_memory.items():
                    ...
                    mem_out = self.forward(self.task_mem_cache[t]['datasets'].to(self.config.device),self.task_mem_cache[t]['task'])
